...
exports.save = async (req, res, next) => {
    db.run(query.insert,
        ...
        , req.body.hora
        , (err) => {
            if (err) { res.status(400).json({ msg: "Não foi possível cadastrar", status: 400, erro: err });  }
...
exports.update = async (req, res, next) => {
    db.run(query.update,
        ...
        , req.body.id
        , (err) => {
            if (err) { res.status(400).json({ msg: "Não foi possível atualizar", status: 400, erro: err });  }
